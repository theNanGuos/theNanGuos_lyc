...
MVP 的 M1–M6 均已达到当前计划的验收门槛。M7 Agent 实验报告已开始，当前进入正文与实验取证阶段；该任务只整理现有实现和新鲜验证证据，不改变生产 Agent 行为，也不在未授权时调用新的付费供应商。
...
| M7 Agent 实验报告 | in_progress（进行中） | 报告正文已改为自然作者视角；助教验收 `code/` 已导出并完成 Windows 独立全栈验证 | 图表导出、个人信息、PDF 视觉验收 |

## 3. 当前 WU

当前进行 `WU-REPORT-001`，撰写实验七 Agent 报告。

- 当前任务：依据已确认的报告设计完成正文、图表、实验与 PDF。
...
| [WU-REPORT-001](work-units/WU-REPORT-001.md) | in_progress（进行中） | Markdown 风格调整完成；精简代码包含 Windows/Linux 指令，后端 123、前端 32、类型/构建、E2E 6 已在 Windows 验证；待 PDF 收束 |

## 5. 最新验证证据

M7 报告证据：

- 正文：`docs/report/实验七-Agent实验报告.md` 已调整为自然作者视角，第一人称仅用于关键选择和实验边界；客观协议、真实案例数据、图片和引用保持不变，用户主动删除的小节未恢复。
- 风格检查：机械套话 `本实验旨在`、`本系统实现了`、`实验表明`、`综上所述` 均为 0；5 张图片路径和 11 条参考资料完整。
- 验收代码包：`code/README.md` 提供 Windows/Linux 的安装、无密钥 Mock、自动化、真实供应商和 CLI 指令；包内后端 123 项、前端 32 项、类型检查、708 模块构建和 Chromium E2E 6 项均在 Windows 独立目录通过。Linux 指令已按跨平台入口编写，但未在 Linux 实机复跑。
- 代码包安全：最终 95 个文件、约 12.3 MB；不含 `.env`、`.venv`、`node_modules`、构建/测试缓存、运行时偏好、生成作品或疑似密钥，后端与知识源（排除派生索引）复制内容一致。
- 可复现证据：`experiments/report/collect_deterministic_evidence.py` 离线生成脱敏 JSON；`tests/test_report_evidence.py` 验证知识评分、角色隔离、GraphState 不跨运行和偏好覆盖顺序。
- 后端全量：2026-07-16 执行 `uv run pytest -q`，132 项通过，2 warnings；`pytest --collect-only -qq` 汇总确认 132 项。
- 既有前端新鲜证据：Vitest 32 项、类型检查、生产构建通过；Chromium E2E 6 项通过（28.6 秒），测试端口无残留监听。
...
1. 完成 `WU-REPORT-001` 的核心正文、架构图和确定性实验取证。
2. 向用户取得姓名、学号并确认最终提交文件名。
3. 生成报告 PDF，逐页修复排版并完成交付验收。
4. 若计划公网部署，必须先增加认证、授权、CSRF/限流、可信反向代理和更强的网络出口控制，不能沿用当前本地单用户安全边界。
