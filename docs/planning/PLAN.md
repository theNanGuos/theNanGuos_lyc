...
目标：依据课程评分项、当前实现和新鲜实验数据，交付以多 Agent、推理、记忆和工具调用为重点的实验报告。

产物：报告 Markdown、架构与实验图表、可复现实验脚本和最终 PDF。

验收门槛：课程必备章节完整；评分项均有设计、代码和实验证据；不虚构隐藏推理或实验结果；PDF 逐页视觉检查通过。
...
| M7 Agent 实验报告 | Markdown、实验数据、图表和逐页验收后的 PDF 完整交付 | P11 |
...
| [WU-REPORT-001](work-units/WU-REPORT-001.md) | P11 | Agent 实验报告 Markdown、证据、图表和 PDF | M1-M6 当前实现与验收证据 |
...
- P11 不修改生产 Agent 行为，也不在未授权时产生新的付费供应商调用。
